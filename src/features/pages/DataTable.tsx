...
import { Button, Checkbox, Popover, Space, Table, Tooltip, type TableColumnsType, type TableProps } from "antd";
...
type TableRow = { key: number; row: string[]; rowIndex: number } & Record<string, string | number | string[]>;
...
export function DataTable({ columns, rows, columnMeta = EMPTY_COLUMN_META, defaultSort, batchActions = EMPTY_BATCH_ACTIONS, selectedRows = EMPTY_SELECTED_ROWS, onSelectionChange, onBatchAction, onSortChange, onView, onEdit, onDelete }: Props) {
  ...
  const visibleColumns = useMemo(
    () => columns.map((column, index) => ({ column, index })).filter(({ column }) => !/^(操作|动作|actions?|operations?)$/i.test(column.trim())),
    [columns],
  );
  ...
  const records = useMemo<TableRow[]>(() => rows.map((row, rowIndex) => ({
    key: rowIndex,
    row,
    rowIndex,
  ...
  const tableColumns = useMemo<TableColumnsType<TableRow>>(() => {
    const result: TableColumnsType<TableRow> = displayedColumns.map(({ column, index }) => {
      ...
      const sortOrder: "ascend" | "descend" | undefined = sort?.column === column ? (sort.order === "asc" ? "ascend" : "descend") : undefined;
      return {
        title: column,
        dataIndex: column,
        key: column,
    ...
    if (hasActions) {
      result.push({
        ...
        fixed: "right",
        width: 116,
        render: (_: unknown, record: TableRow) => <Space size={2} className="generated-table-actions">
          {onView && <Tooltip title={language === "zh" ? "查看" : "View"}><Button type="text" size="small" icon={<Eye size={15} />} aria-label={language === "zh" ? "查看" : "View"} onClick={() => onView(record.row, record.rowIndex)} /></Tooltip>}
          {onEdit && <Tooltip title={language === "zh" ? "编辑" : "Edit"}><Button type="text" size="small" icon={<Pencil size={15} />} aria-label={language === "zh" ? "编辑" : "Edit"} onClick={() => onEdit(record.row, record.rowIndex)} /></Tooltip>}
          {onDelete && <Tooltip title={language === "zh" ? "删除" : "Delete"}><Button danger type="text" size="small" icon={<Trash2 size={15} />} aria-label={language === "zh" ? "删除" : "Delete"} onClick={() => onDelete(record.row, record.rowIndex)} /></Tooltip>}
        </Space>,
      });
    }
    return result;
  ...
  const columnSelector = <Checkbox.Group value={visibleColumns.filter(({ index }) => !hiddenColumns.has(index)).map(({ index }) => index)} onChange={(checked) => {
    const allowed = new Set(checked.map(Number));
    if (allowed.size === 0) return;
    setHiddenColumns(new Set(visibleColumns.filter(({ index }) => !allowed.has(index)).map(({ index }) => index)));
  }} options={visibleColumns.map(({ column, index }) => ({ label: column, value: index }))} className="generated-column-selector" />;

  return <div className="generated-table-shell">
    <div className="generated-table-toolbar">
      {hasSelection && <Space size={8} wrap className="batch-toolbar"><span>{language === "zh" ? `已选 ${selectedRows.size} 条` : `${selectedRows.size} selected`}</span>{batchActions.map((action) => <Button key={action.operation_id} danger={action.method === "DELETE"} disabled={!selectedRows.size} onClick={() => onBatchAction?.(action, [...selectedRows])}>{action.operation_id}</Button>)}</Space>}
      <Popover content={columnSelector} trigger="click" placement="bottomRight"><Button icon={<Columns3 size={15} />}>{language === "zh" ? "配置列" : "Columns"}</Button></Popover>
    </div>
    <Table<TableRow>
