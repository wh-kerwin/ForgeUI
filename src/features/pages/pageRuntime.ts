...
const COMMON_LIST_CONTROL_PARAMETERS = new Set(["page", "pagesize", "limit", "offset", "cursor", "sort", "order", "orderby"]);

export function listFilterParameters(queryParameters: readonly string[] | undefined, pageParam: string, sizeParam: string, sortParam?: string) {
  if (queryParameters === undefined) return undefined;
  const controls = new Set([pageParam, sizeParam, sortParam ?? ""].map((parameter) => parameter.toLocaleLowerCase()));
...
export function queryFilters(filters: Record<string, string>, queryParameters?: readonly string[]) {
  const allowed = queryParameters?.length ? new Map(queryParameters.map((name) => [name.toLocaleLowerCase(), name])) : null;
  return Object.fromEntries(Object.entries(filters).flatMap(([label, value]) => {
...
export function buildListQuery({
  ...
  const query = { ...queryFilters(filters, queryParameters), [pageParam]: String(page), [sizeParam]: String(size) };
