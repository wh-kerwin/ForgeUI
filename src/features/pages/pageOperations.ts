...
export function runtimeOperationFromKey(
  key: string,
  apiDocumentId?: string,
  ...
  if (extra.length || separator < 1 || !supportedMethods.includes(method) || !path || !operationId)
    return null;
  return {
    key,
    method,
...
export function runtimeOperations(
  ...
  return operationKeys.flatMap((key) => {
    ...
    const operation = runtimeOperationFromKey(
      key,
      apiDocumentId,
      operationId ? queryParameters?.[operationId] : undefined,
    );
...
export function runtimeOperation(
  binding: Pick<OperationBinding, "method" | "path" | "operation_id" | "apiDocumentId">,
): RuntimeOperation {
  return {
...
export function pageOperations(page: PageSpec, importedOperations: RuntimeOperation[]) {
  ...
  return page.operations.map((binding) =>
    resolveDocumentBinding(runtimeOperation(binding), importedOperations),
  );
}

function resolveDocumentBinding(
  operation: RuntimeOperation,
  importedOperations: RuntimeOperation[],
) {
  const matches = importedOperations.filter(
    (candidate) =>
      candidate.key === operation.key &&
      candidate.apiDocumentId &&
      (!operation.apiDocumentId || candidate.apiDocumentId === operation.apiDocumentId),
  );
  if (matches.length !== 1) return operation;
  return {
    ...operation,
    apiDocumentId: matches[0].apiDocumentId,
    queryParameters: matches[0].queryParameters,
  };
}
...
export function operationForRole(
  page: PageSpec,
  importedOperations: RuntimeOperation[],
  role: string,
  method: string,
...
function requestedResource(text: string) {
  ...
  return Object.entries(RESOURCE_ALIASES).find(([, aliases]) =>
...
function genericRequestedResource(text: string) {
  ...
  const withoutGenerationPrefix = firstSentence
    .replace(
      /^(?:请)?(?:帮我)?(?:生成|创建|构建|制作|设计|build|create|generate)\s*(?:(?:一个|一套)|an?\s+|the\s+)?/i,
      "",
    )
    .trim();
  const match = withoutGenerationPrefix.match(
    /^(.{1,40}?)(?:管理(?:界面|页面|页)?|列表(?:界面|页面|页)?|详情(?:界面|页面|页)?|数据(?:看板|总览)|看板|界面|页面|页|dashboard|\s+(?:management|list|detail|dashboard|page|screen))/i,
  );
...
function supportsCapability(operation: RuntimeOperation, capability: RequiredCapability) {
  ...
  if (capability === "update")
    return (operation.method === "PUT" || operation.method === "PATCH") && hasPathParameter;
...
function missingCapabilities(prompt: string, documents: readonly ApiDocument[]) {
  ...
  return requiredCapabilities(prompt).filter(
    (capability) => !operations.some((operation) => supportsCapability(operation, capability)),
  );
}
...
export function documentsForPrompt(
  prompt: string,
  documents: readonly ApiDocument[],
): { documents: ApiDocument[]; error?: string } {
  const resource = requestedResource(prompt);
  const genericResource = genericRequestedResource(prompt);
  const resourceLabel = requestedResourceLabel(prompt);
  if (!resourceLabel) return { documents: [...documents] };
  const aliases =
    resource && (!genericResource || RESOURCE_ALIASES[resource].includes(genericResource))
      ? RESOURCE_ALIASES[resource]
      : [resourceLabel];
  const matched = documents.filter((document) =>
    documentMatchesResource(document, resourceLabel, aliases),
  );
  if (!matched.length) {
    return {
      documents: [],
      error: `请求的“${resourceLabel}”没有可识别的对应 API 接口，已停止生成。请导入或勾选该业务实体的 API 文档后重试。`,
    };
  }
  ...
  if (missing.length) {
    return {
      documents: [],
      error: `“${resourceLabel}”接口不具备所需的${missing.map((capability) => CAPABILITY_LABELS[capability]).join("、")}能力，已停止生成。`,
    };
  }
 ...
 */
export function alignListOperationWithPrompt(
  page: PageSpec,
  prompt: string,
  operations: RuntimeOperation[],
): PageSpec {
  ...
  const candidates = operations.filter(
    (operation) =>
      ...
      !/\{[^}]+\}/.test(operation.path) &&
      aliases.some((alias) =>
        `${operation.path} ${operation.operationId}`.toLocaleLowerCase().includes(alias),
      ),
  );
  if (candidates.length !== 1) return page;

  const preferred = candidates[0];
  const listBinding = page.operations?.find(
    (operation) => operation.role === "list" && operation.method === "GET",
  );
  if (
    listBinding?.operation_id === preferred.operationId &&
    listBinding.path === preferred.path &&
    listBinding.apiDocumentId === preferred.apiDocumentId
  )
    return page;
  const nextBinding = {
    apiDocumentId: preferred.apiDocumentId,
    operation_id: preferred.operationId,
    method: preferred.method,
    path: preferred.path,
    role: "list" as const,
  };
  const operationsWithoutList = (page.operations ?? []).filter(
    (operation) => operation !== listBinding,
  );
