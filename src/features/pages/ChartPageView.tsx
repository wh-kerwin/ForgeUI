...
function chartData(
  ...
  if (xIndex < 0 || yIndex < 0)
    return {
      error: `找不到图表字段：${view.xAxisColumn} / ${view.yAxisColumn}`,
      source: [] as ChartDatum[],
      series: [] as string[],
...
export function ChartPageView({
  ...
  const common = (
    <>
      ...
      <XAxis
        dataKey="label"
        tick={{ fill: "var(--fg-text-muted, #8290a3)", fontSize: 11 }}
        axisLine={false}
        tickLine={false}
      />
      <YAxis
  ...
  return (
    <section className="page-chart-view" aria-label={view.title}>
      <div className="chart-heading">
        <h4>{view.title}</h4>
        <span>
          {view.chartType === "pie" ? `${source.length} 项构成` : `${source.length} 条数据`}
        </span>
      </div>
