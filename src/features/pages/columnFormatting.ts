...
export function formatColumnValue(value: string, meta?: ColumnMeta): string {
  ...
  if (meta.type === "money") {
    ...
    return Number.isFinite(number) ? new Intl.NumberFormat(undefined, { style: "currency", currency: currencyForFormat(meta.format), minimumFractionDigits: 2 }).format(number) : value;
  }
  if (meta.type === "number") {
    const number = Number(value);
  ...
  if ((meta.type === "date" || meta.type === "datetime") && value) {
    ...
    if (!Number.isNaN(date.getTime())) return meta.type === "date" ? date.toLocaleDateString() : date.toLocaleString();
  }
