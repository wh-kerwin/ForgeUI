...
export function DatabaseBackupManager({ onNotice }: Props) {
  ...
  async function refresh() {
    ...
    try {
      ...
      onNotice(`读取数据库备份失败：${toUserMessage(error)}`);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function restore(fileName: string) {
    if (!window.confirm(`恢复备份「${fileName}」？当前数据会先自动备份，客户端建议随后重启。`))
      return;
    setLoading(true);
    try {
      ...
      onNotice(`恢复数据库失败：${toUserMessage(error)}`);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <>
      <button
        className="backup-manage-button"
        aria-label={label}
        title={label}
        ...
        <div className="modal-backdrop" onClick={() => setOpen(false)}>
          <section className="modal backup-manager" onClick={(event) => event.stopPropagation()}>
            <div className="modal-head">
              <div>
                <span className="eyebrow">LOCAL BACKUPS</span>
                <h2>数据库备份</h2>
              </div>
            ...
            <p className="modal-intro">
              恢复前会自动创建当前数据库的安全备份。恢复操作不会读取或上传任何业务数据。
            </p>
            ...
            <div className="backup-list">
              ...
              {backups.map((backup) => (
                <div className="backup-entry" key={backup.fileName}>
                  <div>
                    <strong>{backup.fileName}</strong>
                    <span>
                      {formatDate(backup.modifiedAt)} · {formatBytes(backup.sizeBytes)}
                    </span>
                  </div>
                  <button
                    className="secondary"
