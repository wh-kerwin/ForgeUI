...
import { alignListOperationWithPrompt, documentsForPrompt, operationForRole, pageApiDocumentIds, runtimeOperations } from "../pages/pageOperations";
...
export function Workbench() {
  ...
  const generated = useGeneratedPageActions({ projectId: workspace.activeProjectId, apiDocuments: workspace.documents, onNotice: setNotice });
  const persistence = useWorkbenchPersistence({ projectId: workspace.activeProjectId, onNotice: setNotice });
  ...
  const runtimeDocumentIds = new Set(activePageDocumentIds.length ? activePageDocumentIds : workspace.selectedDocumentIds);
  const runtimeDocuments = workspace.documents.filter((document) => document.enabled && runtimeDocumentIds.has(document.id));
  const availableOperations = runtimeDocuments.flatMap((document) => runtimeOperations(document.auth.authorizedOperations ?? [], document.id, document.spec.queryParameters));
  const fieldSchemas = Object.assign({}, ...runtimeDocuments.map((document) => document.spec.fieldSchemas ?? {}));
  ...
  async function generate() {
    ...
    const documentSelection = documentsForPrompt(prompt, workspace.selectedDocuments.filter((document) => document.enabled));
    if (!documentSelection.documents.length) return setNotice(documentSelection.error ?? "请至少选择一个启用的 API 文档");
    const generationDocuments = documentSelection.documents;
    const generationOperations = generationDocuments.flatMap((document) => runtimeOperations(document.auth.authorizedOperations ?? [], document.id, document.spec.queryParameters));
    setGenerating(true);
    ...
    try {
      ...
      try {
        page = await requestPage(buildModelRequest(active, prompt, generationDocuments, parseTemplate(selectedTemplate), selectedPromptTemplate));
      } catch (error) {
        ...
        try {
          ...
          page = await requestPage(buildModelRequest({ ...active, streaming: false, structuredOutput: "jsonSchema" }, prompt, generationDocuments, parseTemplate(selectedTemplate), selectedPromptTemplate));
        } catch {
      ...
      const apiDocumentIds = pageApiDocumentIds(page, generationDocuments.map((document) => document.id));
      generated.setPage(page);
      setActivePageDocumentIds(apiDocumentIds);
      await persistence.saveSession(active.id, prompt, page, apiDocumentIds);
      const selectedRuntimeOperations = generationOperations.filter((operation) => apiDocumentIds.includes(operation.apiDocumentId ?? ""));
      const listOperation = page.operations?.some((operation) => operation.role === "list" && operation.method === "GET") ? operationForRole(page, selectedRuntimeOperations, "list", "GET") : undefined;
      setNotice(usedOpenApiFallback ? "模型输出无法解析，已根据所选 OpenAPI 生成基础页面" : "已生成真实模型 PageSpec 页面");
  ...
  async function refinePage(instruction: string) {
    ...
    try {
      ...
      const refinedPage = await requestPage(buildModelRequest(active, refinementPrompt, workspace.documents.filter((document) => refinementIds.has(document.id)), current, selectedPromptTemplate));
      const refinementOperations = workspace.documents.filter((document) => refinementIds.has(document.id) && document.enabled).flatMap((document) => runtimeOperations(document.auth.authorizedOperations ?? [], document.id, document.spec.queryParameters));
      const page = alignListOperationWithPrompt(refinedPage, `${current.title} ${instruction}`, refinementOperations);
  ...
  const generatePage = <main className="route-main"><PromptGenerator prompt={prompt} onPromptChange={setPrompt} onGenerate={generate} generating={generating} onOpenSettings={() => navigate("models")} templates={persistence.templates} selectedTemplateId={selectedTemplateId} onTemplateSelect={setSelectedTemplateId} onTemplateClear={() => setSelectedTemplateId("")} promptTemplates={promptTemplates} selectedPromptTemplateId={selectedPromptTemplateId} onPromptTemplateSelect={setSelectedPromptTemplateId} onManagePromptTemplates={() => navigate("models")} apiDocuments={workspace.documents} selectedApiDocumentIds={workspace.selectedDocumentIds} onApiDocumentSelectionChange={workspace.setSelectedDocumentIds} /><GeneratedWorkbenchView page={streamingPage ?? generated.page} projectId={workspace.activeProjectId} apiDocumentIds={activePageDocumentIds} isStreaming={isStreaming} fieldSchemas={fieldSchemas} grantedRoles={workspace.grantedRoles} modelId={active?.id} templateId={selectedTemplateId || undefined} templateName={selectedTemplate?.name} operations={availableOperations} detail={generated.detail} onDetail={generated.loadDetail} onSaved={persistence.refreshTemplates} onQuery={generated.query} onMutation={generated.mutate} onDelete={generated.deleteRecord} querying={generated.querying} queryMeta={generated.queryMeta} onRefine={refinePage} refining={refining} sessions={persistence.sessions} templates={persistence.templates} versions={persistence.versions} selectedTemplateId={persistence.versionTemplateId} onOpenSession={openSession} onDeleteSession={persistence.removeSession} onUseTemplate={useTemplate} onShowVersions={persistence.showVersions} onRestore={persistence.restoreVersion} onInvalid={setNotice} onExport={persistence.exportTemplate} onImport={persistence.importTemplate} onDeleteTemplate={persistence.removeTemplate} onRenameTemplate={persistence.renameSavedTemplate} /></main>;
  const routeViews: Record<AppRoute, ReactNode> = {
    overview: <OverviewPage templates={persistence.templates} sessions={persistence.sessions} spec={workspace.activeDocument?.spec} modelReady={isModelConfigured(active)} onNavigate={navigate} onStartGenerate={(nextPrompt = "") => { setPrompt(nextPrompt); setSelectedTemplateId(""); navigate("generate"); }} />,
    ...
    business: <BusinessPage documents={workspace.documents} activeDocument={workspace.activeDocument} auth={workspace.activeDocument?.auth ?? null} secret={workspace.secret} onDocumentSelect={workspace.setActiveDocumentId} onAuthChange={workspace.setAuth} onSecretChange={workspace.setSecret} onSave={workspace.saveAuth} />,
    openapi: <OpenApiPage documents={workspace.documents} activeDocumentId={workspace.activeDocumentId} auth={workspace.activeDocument?.auth ?? null} onSelect={workspace.setActiveDocumentId} onImport={workspace.importSwaggerUrl} onImportFile={workspace.importOpenApiFile} onAuthChange={workspace.setAuth} onSave={workspace.saveAuth} onRename={workspace.renameDocument} onToggle={workspace.toggleDocument} onDelete={workspace.deleteDocument} />,
  ...
  return <div className="app"><WorkbenchSidebar route={route} onNavigate={navigate} onNotice={setNotice} projects={workspace.projects} activeProjectId={workspace.activeProjectId} onProjectSelect={workspace.selectProject} onProjectCreate={workspace.createProject} onProjectRename={workspace.renameProject} onProjectDelete={workspace.deleteProject}/>{page}{notice && <div className="toast">{notice}<button onClick={() => setNotice("")}><X size={14}/></button></div>}</div>;
}
