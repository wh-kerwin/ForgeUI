  ...
  return {
    ...
    ...(field.visibleWhen
      ? {
          visibleWhen: {
            ...field.visibleWhen,
            equals: Array.isArray(field.visibleWhen.equals)
...
function bindingForDocument(document: ApiDocument): OperationBinding[] {
  return toAllowedOperations(
    document.spec,
    document.id,
    document.auth.authorizedOperations ?? [],
  ).map((operation) => {
    const role = inferOperationRoles([operation])[operation.operation_id]?.[0] as
      OperationBinding["role"] | undefined;
...
function schemaFields(
  ...
  operations: readonly OperationBinding[],
): FieldSchema[] {
  const operationKeys = new Set(
    operations.map((operation) => `${operation.apiDocumentId}\0${operation.operation_id}`),
  );
...
export function createApiFallbackPage(
  prompt: string,
  selectedDocuments: readonly ApiDocument[],
): PageSpec {
  ...
  if (!selection.documents.length)
    throw new Error(selection.error ?? "没有可用于生成页面的 API 文档");
  ...
  const columns = [
    "id",
    ...fields.map((field) => field.name).filter((name) => name !== "id"),
  ].slice(0, MAX_COLUMNS);
  const documentNames = documents
    .map((document) => document.spec.title.trim() || document.name.trim())
    .filter(Boolean);
      ...
      ...(fieldByName.get(name)?.type === "enum"
        ? {
            enumLabels: Object.fromEntries(
              (fieldByName.get(name)?.enumValues ?? []).map((value) => [value, value]),
            ),
          }
        : {}),
    })),
