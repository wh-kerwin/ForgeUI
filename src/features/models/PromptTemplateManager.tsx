...
export function PromptTemplateManager({
  ...
  const save = () => {
    ...
    onChange(
      draft.id
        ? custom.map((item) => (item.id === draft.id ? template : item))
        : [...custom, template],
    );
    setDraft(null);
  };
  const remove = (id: string) => {
    if (window.confirm(zh ? "删除这个自定义 Prompt 模板？" : "Delete this custom prompt template?"))
      onChange(custom.filter((item) => item.id !== id));
  };
  return (
    <section className="prompt-template-manager">
      <div className="section-heading">
        <div>
          <span className="eyebrow">PROMPT TEMPLATES</span>
          <h2>{zh ? "系统提示词模板" : "System prompt templates"}</h2>
        </div>
        <button type="button" className="secondary" onClick={() => setDraft(emptyDraft())}>
          <Plus size={15} />
          {zh ? "新增模板" : "Add template"}
        </button>
      </div>
      <div className="prompt-template-list">
        {[...builtIns, ...custom].map((template) => (
          <div className="prompt-template-row" key={template.id}>
            ...
            {!template.isDefault && template.id.startsWith("custom-") && (
          ...
          <div className="two-col">
            ...
            <label>
              ...
              <SelectField
                ...
                onChange={(value) => set("scene", value as PromptTemplate["scene"])}
                ariaLabel={zh ? "场景" : "Scene"}
              />
            </label>
          </div>
          <label>
