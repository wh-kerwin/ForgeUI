...
export function ModelsPage({ models, active, selectedId, onSelect, onEdit, onAdd, onDuplicate, onMakeDefault, onDelete, editor, editorModel, onClose, onSave, onTest, testing }: any) {
  ...
  return <PageLayout eyebrow="MODEL SERVICES" title={zh ? "模型配置" : "Model services"} intro={zh ? "管理多个 OpenAI Compatible 或 Anthropic Compatible 模型服务。" : "Manage multiple OpenAI Compatible or Anthropic Compatible model services."}>
    ...
    {editor && <ModelEditor initial={editorModel} onClose={onClose} onSave={onSave} onTest={onTest} testing={testing}/>}
  </PageLayout>;
}
