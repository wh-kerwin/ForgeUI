...
export function ModelsPage({
  ...
  return (
    <PageLayout
      ...
      title={zh ? "模型配置" : "Model services"}
      intro={
        zh
          ? "管理多个 OpenAI Compatible 或 Anthropic Compatible 模型服务。"
          : "Manage multiple OpenAI Compatible or Anthropic Compatible model services."
      }
    >
      <div className="models-toolbar">
        <span className="muted">
          {zh
            ? `已配置 ${models.length} 个模型服务 · 当前使用：${active?.name || "未配置"}`
            : `${models.length} model service${models.length === 1 ? "" : "s"} configured · Active: ${active?.name || "Not configured"}`}
        </span>
      ...
      <PromptTemplateManager
        builtIns={builtInPromptTemplates}
      ...
      {editor && (
        <ModelEditor
          initial={editorModel}
          promptTemplates={[...builtInPromptTemplates, ...customPromptTemplates]}
          onClose={onClose}
          onSave={onSave}
          onTest={onTest}
          testing={testing}
