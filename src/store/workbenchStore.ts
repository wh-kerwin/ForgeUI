...
export function resolveApiDocument(
  projectId: string,
  apiDocuments: ApiDocument[],
  ...
  if (operation.apiDocumentId) {
    ...
    if (!document)
      return { ok: false, error: "当前项目中找不到页面绑定的 API 文档，请重新绑定接口" };
    ...
    const matches = projectDocuments.filter((candidate) =>
      candidate.auth.authorizedOperations?.includes(operation.key),
    );
    if (matches.length === 0)
      return { ok: false, error: `当前项目中没有授权接口“${operation.key}”的 API 文档` };
    if (matches.length > 1)
      return {
        ok: false,
        error: `旧页面的接口“${operation.key}”匹配到多个 API 文档，请重新生成页面或重新绑定接口`,
      };
    document = matches[0];
  }
  if (!document.enabled)
    return { ok: false, error: `API 文档“${document.name}”已停用，请先启用后重试` };
...
export const useWorkbenchStore = create<WorkbenchState>((set, get) => ({
  ...
  apiContext: null,
  setPage: (page) =>
    set((state) => ({ page: typeof page === "function" ? page(state.page) : page })),
  ...
  query: async (filters = {}, operation) => {
    ...
    try {
      const response = await request(
        resolved.context,
        resolved.document,
        `${resolved.url}${queryString ? `?${queryString}` : ""}`,
        resolved.operation,
      );
      if (sequence !== querySequence) return;
      ...
      set((state) => ({
        page: state.page
          ? {
              ...
              columnMeta: state.page.columnMeta?.filter((meta) =>
                mapped.columns.includes(meta.name),
              ),
              rows: mapped.rows,
            }
          : state.page,
        queryMeta: { total: mapped.total },
      }));
      resolved.context.onNotice(`已从真实业务 API 加载 ${mapped.rows.length} 条数据`);
    } catch (error) {
      if (sequence === querySequence) resolved.context.onNotice(toUserMessage(error));
    } finally {
  ...
  loadDetail: async (operation, id) => {
    ...
    try {
      const response = await request(
        resolved.context,
        resolved.document,
        resolved.url.replace(/\{[^}]+\}/, encodeURIComponent(id)),
        operation,
      );
      set({
        detail:
      ...
      resolved.context.onNotice(toUserMessage(error));
    }
  },
  mutate: async (operation, text) => {
    ...
    try {
      ...
      return resolved.context.onNotice("请求体必须是合法 JSON");
    }
