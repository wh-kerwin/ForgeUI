...
test("promotes the real record ID even when the API serializes it after display fields", () => {
  const mapped = mapRealDataResponse([{ category: "电子产品", description: "Apple", id: 81, name: "iPhone" }]);
...
test("keeps an API pagination total for the generated page pager", () => {
  assert.deepEqual(mapRealDataResponse({ data: [{ id: 7 }], total: 42 }), { columns: ["id"], rows: [["7"]], total: 42 });
});
