...
import { buildListQuery, firstListSort, generatedActionPath, hasPageAccess, isGeneratedActionPath, listFilterParameters, parseGrantedRoles, queryFilters } from "../src/features/pages/pageRuntime";
...
test("filter labels map only to declared OpenAPI query parameters", () => {
  assert.deepEqual(queryFilters(
    { "姓名": "周设计师", "部门": "设计部", "状态": "在职" },
    ["page", "pageSize", "department", "keyword", "sort"],
...
test("list filters are exactly the non-control parameters declared by the OpenAPI operation", () => {
  assert.deepEqual(
    listFilterParameters(["page", "pageSize", "keyword", "category", "minPrice", "maxPrice", "sort"], "page", "pageSize"),
