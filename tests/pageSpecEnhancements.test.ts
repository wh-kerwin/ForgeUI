...
test("recursive tabs and split views parse with column and batch metadata", () => {
  const page = parsePageSpec({
        ...
        items: [
          {
            ...
            view: {
              type: "split",
              splitRatio: 0.6,
              left: { type: "list", defaultSort: { column: "amount", order: "desc" } },
              right: {
                type: "chart",
                title: "Amounts",
                chartType: "bar",
    ...
    batchActions: [
      {
        operation_id: "archiveOrders",
        method: "POST",
        path: "/orders/archive",
        payloadBuilder: { type: "ids" },
      },
    ],
  });
  assert.ok(page);
...
test("invalid recursive views and incomplete custom batch payloads are rejected", () => {
  ...
  assert.equal(
    parsePageSpec({
      ...base,
      views: [{ type: "split", splitRatio: 0.95, left: { type: "list" }, right: { type: "list" } }],
    }),
    null,
  );
  assert.equal(
    parsePageSpec({
      ...base,
      batchActions: [
        { operation_id: "x", method: "POST", path: "/x", payloadBuilder: { type: "custom" } },
      ],
    }),
    null,
  );
...
test("page metadata, documented operation roles, and linked body schemas parse", () => {
          ...
          bodySchema: [
            {
              ...
              type: "string",
              required: true,
              visibleWhen: { field: "status", equals: [] },
            },
          ],
        },
      ],
  ...
  assert.equal(
    parsePageSpec({
      ...base,
      operations: [
        { apiDocumentId: 42, operation_id: "x", method: "GET", path: "/x", role: "list" },
      ],
    }),
    null,
  );
...
test("theme presets merge custom tokens and expose stable CSS variables", () => {
  ...
  assert.equal(
    parsePageSpec({
      ...base,
      theme: "custom",
      styleTokens: { primary: "url(https://example.com/pixel)" },
    }),
    null,
  );
  assert.equal(
    parsePageSpec({ ...base, theme: "custom", styleTokens: { text: "#000000e0" } })?.styleTokens
      ?.text,
    "#000000e0",
  );
});
