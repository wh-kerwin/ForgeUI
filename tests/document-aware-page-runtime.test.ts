...
import {
  ...
  pageApiDocumentIds,
  pageOperations,
  runtimeOperationFromKey,
  runtimeOperations,
  type RuntimeOperation,
} from "../src/features/pages/pageOperations";
...
test("authorized operation keys become document-scoped runtime operations", () => {
  assert.deepEqual(runtimeOperations([key], "employees-api"), [
    { ...operation, apiDocumentId: "employees-api" },
  ]);
...
test("selected document scope repairs a unique legacy model binding", () => {
  const page = {
    ...
    rows: [],
    operations: [
      { operation_id: "listEmployees", method: "GET", path: "/employees", role: "list" },
    ],
  } satisfies PageSpec;
  const selectedOperations = runtimeOperations([key], "employees-api");

  assert.equal(
    operationForRole(page, selectedOperations, "list", "GET")?.apiDocumentId,
    "employees-api",
  );
...
test("list fallback skips detail GET operations regardless of document order", () => {
  ...
  const selectedOperations = runtimeOperations(
    [operationKey("GET", "/employees/{id}", "getEmployee"), key],
    "employees-api",
  );

  assert.equal(
    operationForRole(page, selectedOperations, "list", "GET")?.operationId,
    "listEmployees",
  );
  assert.equal(
    operationForRole(page, selectedOperations, "detail", "GET", true)?.operationId,
    "getEmployee",
  );
});

test("an order prompt replaces a mismatched customer list binding", () => {
  const customerList: RuntimeOperation = {
    key: operationKey("GET", "/customers", "listCustomers"),
    method: "GET",
    path: "/customers",
    operationId: "listCustomers",
    apiDocumentId: "customer-api",
  };
  const orderList: RuntimeOperation = {
    key: operationKey("GET", "/orders", "listOrders"),
    method: "GET",
    path: "/orders",
    operationId: "listOrders",
    apiDocumentId: "order-api",
  };
  const page = {
    title: "订单管理",
    description: "",
  ...
  assert.deepEqual(aligned.operations?.[0], {
    apiDocumentId: "order-api",
...
test("an explicit order request only supplies the matching selected API document", () => {
  const customers = document({
    ...
    spec: {
      ...document().spec,
      title: "Customers",
      operations: [operationKey("GET", "/customers", "listCustomers")],
    },
  });
  const orders = document({
    ...
    spec: {
      ...document().spec,
      title: "Orders",
      operations: [operationKey("GET", "/orders", "listOrders")],
    },
  });
  assert.deepEqual(
    documentsForPrompt("生成订单管理", [customers, orders]).documents.map((item) => item.id),
    ["orders-api"],
  );
...
test("a generic business resource is matched from its imported API document", () => {
  const supplier = document({
    ...
    spec: {
      ...document().spec,
      title: "供应商服务",
      operations: [operationKey("GET", "/suppliers", "listSuppliers")],
    },
    auth: {
      ...document().auth,
      authorizedOperations: [operationKey("GET", "/suppliers", "listSuppliers")],
    },
  });

  assert.deepEqual(
    documentsForPrompt("生成供应商管理界面", [supplier]).documents.map((item) => item.id),
    ["suppliers-api"],
  );
...
test("a more specific requested feature cannot fall back to its broader resource API", () => {
  const orders = document({
    ...
    auth: {
      ...document().auth,
      authorizedOperations: [operationKey("GET", "/orders", "listOrders")],
    },
  });
...
test("management requests stop before generation when the matched API lacks a list endpoint", () => {
  const orders = document({
    ...
    spec: {
      ...document().spec,
      title: "Orders",
      operations: [operationKey("GET", "/orders/{id}", "getOrder")],
    },
    auth: {
      ...document().auth,
      authorizedOperations: [operationKey("GET", "/orders/{id}", "getOrder")],
    },
  });
...
test("requested CRUD actions must be present in the matched API document", () => {
  ...
  assert.match(
    documentsForPrompt("生成订单管理界面，支持新增、编辑和删除", [orders]).error ?? "",
    /新增、编辑、删除/,
  );
});
...
test("all documents selected for generation remain attached when only one is emitted", () => {
  ...
  assert.deepEqual(pageApiDocumentIds(page, ["departments-api", "employees-api"]), [
    "departments-api",
    "employees-api",
  ]);
});
...
test("legacy operation resolves only when one current-project document authorizes it", () => {
  ...
  assert.equal(
    resolveApiDocument("project-a", [employeeDocument, otherProjectDocument], operation).document
      ?.id,
    "employees-api",
  );

  const ambiguous = resolveApiDocument(
    "project-a",
    [employeeDocument, document({ id: "duplicate" })],
    operation,
  );
...
test("explicit bindings reject disabled, unauthorized, and cross-project documents", () => {
  ...
  assert.match(
    resolveApiDocument("project-a", [document({ enabled: false })], boundOperation).error ?? "",
    /已停用/,
  );
  assert.match(
    resolveApiDocument(
      "project-a",
      [document({ auth: { ...document().auth, authorizedOperations: [] } })],
      boundOperation,
    ).error ?? "",
    /未授权/,
  );
  assert.match(
    resolveApiDocument("project-b", [document()], boundOperation).error ?? "",
    /找不到页面绑定/,
  );
});
